...
use std::path::Path;
use wasi_common::{file::FileCaps, dir::DirCaps, table::Table, Error, WasiCtx, WasiFile};
...
impl WasiCtxBuilder {
    ...
    pub fn preopened_dir_with_caps(mut self, dir: Dir, guest_path: impl AsRef<Path>, dir_caps: DirCaps, file_caps: FileCaps) -> Result<Self, Error> {
        let dir = Box::new(crate::dir::Dir::from_cap_std(dir));
        self.0.push_preopened_dir_with_caps(dir, guest_path, dir_caps, file_caps)?;
        Ok(self)
    }
